...
class provider implements
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        $sql = "SELECT cm.id AS cmid, ga.vars AS attempt, g.timemodified
                FROM {context} c
                INNER JOIN {course_modules} cm ON cm.id = c.instanceid AND c.contextlevel = :contextlevel
                INNER JOIN {modules} m ON m.id = cm.module AND m.name = :modname
                INNER JOIN {geogebra} g ON g.id = cm.instance
                INNER JOIN {geogebra_attempts} ga ON ga.geogebra = g.id
                WHERE c.id {$contextsql} AND ga.userid = :userid
                ORDER BY cm.id";
        ...
        // because we know the results are ordered, we know when we've moved to the attempt for a new geogebra activity and therefore
